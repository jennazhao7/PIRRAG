...
struct PackedCentroidBlock {
  Plaintext centroidPlain;
  Plaintext centroidNormPlain;
  int centroidsInBlock = 0;
};
...
void WriteCentroidBatchDistanceMetadata(
  ...
  out << "  \"format_version\": \"openfhe_centroid_batch_prepacked_v1\",\n";
  ...
  out << "  \"prepack_seconds\": " << prepackSeconds << ",\n";
  out << "  \"compute_seconds\": " << computeSeconds << "\n";
  out << "}\n";
}
...
int main(int argc, char** argv) {
  try {
    ...
    const bool skipWrites = HasArg(args, "--skip-writes");
    ...
    std::cout << "Prepacking " << nBatches << " plaintext centroid blocks for "
              << nCentroids << " centroids.\n";

    auto prepackStart = std::chrono::high_resolution_clock::now();
    std::vector<PackedCentroidBlock> packedBlocks(static_cast<size_t>(nBatches));
    for (int batchIdx = 0; batchIdx < nBatches; ++batchIdx) {
      ...
      packedBlocks[static_cast<size_t>(batchIdx)].centroidPlain =
          cc->MakeCKKSPackedPlaintext(centroidPacked);
      packedBlocks[static_cast<size_t>(batchIdx)].centroidNormPlain =
          cc->MakeCKKSPackedPlaintext(centroidNormPacked);
      packedBlocks[static_cast<size_t>(batchIdx)].centroidsInBlock = centroidsInBatch;
    }
    const double prepackSeconds =
        std::chrono::duration<double>(
            std::chrono::high_resolution_clock::now() - prepackStart)
            .count();

    std::cout << "Computing centroid-batched distances with prepacked plaintexts"
              << " (centroids-per-ciphertext=" << centroidsPerCiphertext
              << ", padded-dim=" << paddedDim
              << ", threads=" << (numThreads > 0 ? numThreads : omp_get_max_threads())
              << ", skip-writes=" << (skipWrites ? "true" : "false") << ")\n";
    ...
    for (int batchIdx = 0; batchIdx < nBatches; ++batchIdx) {
      ...
      try {
        const auto& block = packedBlocks[static_cast<size_t>(batchIdx)];
        Ciphertext<DCRTPoly> product = cc->EvalMult(encQuery, block.centroidPlain);
        ...
        Ciphertext<DCRTPoly> sumNorms = cc->EvalAdd(encNorm, block.centroidNormPlain);
    ...
    WriteCentroidBatchDistanceMetadata(
        ...
        prepackSeconds,
        computeSeconds);
    std::cout << "Prepack time: " << prepackSeconds << " s\n";
    std::cout << "Computed " << nCentroids << " centroid distances in " << nBatches
              << " packed ciphertexts in " << computeSeconds << " s"
              << " (" << (nCentroids / computeSeconds) << " centroids/s).\n";
    return 0;
  } catch (const std::exception& ex) {
    std::cerr << "openfhe_compute_distances_centroid_prepacked error: " << ex.what() << "\n";
    return 1;
  }
}
